...
# docker-ce: docker社区版本 负责提供docker引擎以及docker容器运行时的所有组件
# docker-ce-cli: 负责提供基于命令行客户端与docker交互
# containerd.io: 负责管理所有容器的生命周期以及镜像的传输和存储和管理网络插件等
# docker-compose-plugin：未来就可以用docker compose这个命令来启动和管理容器
# docker  compose up...
# docker compose down...
# docker compose  log... 
```
...
{
  "registry-mirrors": ["https://dockerproxy.cn","https://docker.1panel.live"]
   #或者
   "registry-mirrors": ["https://vo3pldr7.mirror.aliyuncs.com"]
}
EOF
...
```sh
docker  run -d -p 80:80 --name=nginx-01 --restart=always nginx
```
...
**使用docker compose的使用规则**：

- 1.编写一个yaml（倡导）或者yml文件

- compose.yaml

- 2.在这个文本文件中把应用要用到各个中间件的命令都写到这个文本文件中，还要根据compose的规则来写。

- 3.使用compose文件来启动应用的所有中间件。

```shell
docker compose  -f compose.yaml   up
# 启动这个文件中写好的所有容器
docker compose  -f compose.yaml   down
# 启动这个文件中写好的所有容器停止（stop） && 移除(remove)
...
> 测试号的地址：https://mp.weixin.qq.com/wxamp/sandbox?doc=1
>
> 找测试号给你的AppId  填写好